# Standalone fullscreen helper for development

The current ZIP integrates this functionality into MapleRoyals.app via `portable/GameDisplay.swift`; it no longer includes separate display apps. This directory retains the standalone helper for developer experiments. Players using the compiled ZIP should follow the [fullscreen instructions in the player guide](../docs/PLAYER_GUIDE.md#fullscreen-with-black-side-bars). The notes below are for developers building or changing the helper.
...
The implementation uses private CoreGraphics APIs. Compatibility with other macOS versions, multiple external displays, process crashes, sleep/wake and production signing are not validated. This standalone program remains experimental scaffolding; the portable app now has its own integrated display lifecycle using the same private declarations. The reused declarations and their MIT notice are included here. No game or Wine assets are included.
