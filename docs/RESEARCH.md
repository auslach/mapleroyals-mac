...
**Later update, September 14, 2026:** the portable app now provisions its own runtime and prefix outside the app. Testing the actual ZIP with an absent data directory completed fresh direct-CX24 installation, character loading on retry, map changes and normal shutdown on the original M1 Pro. One initial character-loading failure remains unexplained. Reopening the app skipped setup, launched the installed game, and the user confirmed character entry again. See [ZIP acceptance](ZIP_ACCEPTANCE.md). The workspace-dependent app and recipe limitations below describe the earlier proof of concept.
