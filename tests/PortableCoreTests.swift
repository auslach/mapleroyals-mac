...
struct PortableCoreTests {
    ...
    static func main() throws {
        ...
        let img = temporary.appendingPathComponent("MapleRoyalsSetupImg-test.exe")
        try Data("MZtest".utf8).write(to: img)
        try rejects({ try core.install(img) }, "Reject the wrong official installer format before downloading or changing a prefix")
